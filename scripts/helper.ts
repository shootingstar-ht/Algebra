...
function _createWalletAndDeployer(hre: HardhatRuntimeEnvironment): [Wallet, Deployer] {
  const wallet = new Wallet(secrets.privateKey)
  console.log(`Wallet created ${wallet.address}`)
  return [wallet, new Deployer(hre, wallet)]
}

export function initializeWalletAndDeployer(hre: HardhatRuntimeEnvironment): [Deployer] {
...
export function createArgument(type: ArgumentTypes, contractName?: string | undefined): string {
